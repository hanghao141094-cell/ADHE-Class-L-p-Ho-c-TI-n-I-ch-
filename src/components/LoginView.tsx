  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (selectedRole === 'teacher') {
      ...
      const foundStudent = students.find(s => 
        s.name.toLowerCase() === inputName || 
        (s.studentCode && s.studentCode.toLowerCase() === inputName) ||
        s.parentPhone === inputName
      );
      if (foundStudent) {
        if (foundStudent.isActive === false) {
          setErrorMessage('Tài khoản học sinh này chưa được giáo viên kích hoạt! 🏫 Vui lòng đợi cô giáo phê duyệt danh sách.');
          return;
        }
              ...
              {(selectedRole === 'student' || selectedRole === 'parent') && (
                <div className="bg-slate-100/80 p-1 rounded-xl flex items-center justify-between text-xs font-black text-slate-600 mb-2 border border-slate-200">
                  <button
                    ...
                    className={`flex-1 py-2 text-center rounded-lg transition duration-200 cursor-pointer text-[11px] ${
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setErrorMessage('');
                      setFullName(students[0]?.parentName || 'Nguyễn Thành');
                    }}
                    className={`flex-1 py-2 text-center rounded-lg transition duration-200 cursor-pointer text-[11px] ${
                      selectedRole === 'parent'
                        ? 'bg-white text-indigo-950 font-black shadow-xs'
